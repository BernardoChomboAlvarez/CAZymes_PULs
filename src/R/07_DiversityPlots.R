...
## PULS SUBSTRATE PLOT
plot.puls.sub <- function(all.puls) {
    ## Create an array with the unique values of the 'Family' column
    fam.arr <- c(sort(unique(all.puls$Family)))
    ...
    for (fam in fam.arr) {
        ...
        png(filename = paste0("PULs_",fam,"_SubstrateDiversity.png"),width = 3000,height = 1000,res = 350)
        ...
        plot <- ggplot(data = df.temp,
            ...
            scale_x_discrete(breaks = labels.arr,
                             labels = labels.counts.arr) +
            guides(fill = guide_legend(ncol = 2,title = "Substrate")) +
            labs(caption = paste0("n = ",sum(counts.df$counts)))
...
plot.puls.sub(all.puls = all.puls)
