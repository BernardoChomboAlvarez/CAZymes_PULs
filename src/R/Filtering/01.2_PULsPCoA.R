...
plot.pcoa <- function(mtx,metadata,labels,component) {
    ...
    if (length(labels) <= 25) {
        ...
        text(x= 2.0 , y= -1.35 ,expression(paste("Permanova")), cex = 0.3)
        text(x= 2.0 , y= -1.55 ,paste("p-value = ", pval, sep = ""), cex = 0.3)
...
## Filter by Family
family.arr <- c("Acidaminococcaceae","Bacteroidaceae","Burkholderiaceae","Chitinophagaceae","Enterobacteriaceae","Enterococcaceae","Erysipelatoclostridiaceae","Flavobacteriaceae","Hahellaceae","Lachnospiraceae","Lactobacillaceae","Megasphaeraceae","Moraxellaceae","Paludibacteraceae","Rikenellaceae","Ruminococcaceae","Thermaceae","Veillonellaceae","Weeksellaceae")
MAGS.meta <- MAGS.meta[is.element(MAGS.meta$Family,family.arr),]
