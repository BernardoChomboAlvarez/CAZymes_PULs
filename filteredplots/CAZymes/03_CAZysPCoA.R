...
plot.pcoa <- function(mtx,metadata,labels,component) {
    ...
    if (length(labels) <= 25) {
        ...
        legend((lgend.x-0.6),(legend.y+0.2), cex = 0.3 , component,bty = "n" ,pch = 21,col="white",pt.bg = c("white"))
        text(x= 1.5 , y= -1.0 ,expression(paste("Permanova")), cex = 0.3)
        text(x= 1.5 , y= -1.10 ,paste("p-value = ", pval, sep = ""), cex = 0.3)
...
# ===================================================================
## At the end we will be working with a matrix 343 MAGs x 318 CAZys
# ===================================================================


## UNIVERSAL MATRIX GENERATION
## Use the current data
matrix.data <- all.cazys
